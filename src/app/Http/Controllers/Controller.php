...
class Controller extends BaseController
{
	...
	public function counter(){
        ...
        $count = Statistik::where('ip',$ip)->where('date',$date)->count();
        if($count == 0){
            ...
            $statistik = Statistik::where('ip',$ip)->where('date',$date)->update(array(
                'hits' =>  Statistik::where('ip',$ip)->where('date',$date)->first()->hits +1,
            ));
        }
